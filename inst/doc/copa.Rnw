...
shown by Tomlins et al., as well as others \citet{Fonseca:2004}.
...
given number of outliers using the \Rfunction{table} function.

<<>>=
tab <- rev(table(rslt$pr.sums))
tab
@ 

The \Rfunction{rev} function is used here to order things from larger to smaller.
We can see that there are \Sexpr{tab[[1]]} gene pairs that have \Sexpr{as.numeric(names(tab[1]))}
outliers and \Sexpr{tab[[2]]} gene pairs that have \Sexpr{as.numeric(names(tab[2]))} 
outliers. 
...
\Sexpr{as.numeric(names(tab[1]))} outliers then it is fairly likely that our
